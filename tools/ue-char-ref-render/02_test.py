...
# ============ 配置 ============
OUTPUT_DIR    = r"D:/ref_shots/test"          # 测试输出目录
RESOLUTION    = (1024, 2304)                    # W x H
LEVEL_PATH    = "/Game/Maps/L_CharRefShoot"
SEARCH_DIRS   = ["/Game/ArtResources/Characters/Cst"]  # 你的角色目录
TEST_MESH_HINT = "_LOD0"                         # 找名字含这个的第一个（只要 LOD0）
TEST_CHARACTER  = "Dolores_Const"                # 指定角色名子串（None = 用字母序第一个）
EXCLUDE_PATH_KEYWORDS = ["/Weapon/", "/Sub1/", "/Sub2/", "/Sub3/", "/VFX/", "/Anim/"]
EXCLUDE_NAME_KEYWORDS = ["_Weapon", "_VFX", "_Anim", "PhysicsAsset", "Skeleton", "Sub1_", "Sub2_", "Sub3_"]
...
def shoot_views(actor, name):
    ...
    # 用 LDR 才会经过 tonemap 输出 sRGB 8bit，配合 RGBA8 RT → PNG
...
def post_process_alpha(png_paths):
    """UE PropagateAlpha=2 输出 (premultiplied_rgb, 1-coverage)。
    转成标准 PNG (true_rgb, coverage)：
      1) alpha = 255 - alpha
      2) rgb = rgb / (new_alpha/255)   (unpremultiply)
    """
    if not png_paths:
        return
    script = (
        ...
        "    # 角色区域 a>0，unpremultiply；背景 a=0，rgb 保 0\n"
        "    mask = af > 0\n"
