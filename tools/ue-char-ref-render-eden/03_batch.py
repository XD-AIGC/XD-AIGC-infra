...
WHITELIST_MESH_PATHS = []  # [] = 全量(SEARCH_DIRS 三组)到 OUTPUT_DIR；小批验证时设 _ETH_MIX_TEST

VIEWS = [("front", 180.0), ("side", -90.0), ("back", 0.0), ("tq", 135.0)]
FILL_RATIO = 0.80
SKIP_EXISTING = True   # 全量续传：跳过已渲好的；中途挂了重跑会接着渲
WARMUP_CAPTURES = 6    # 每个 mesh 截图前空跑几次预热（给贴图加载留时间，防黑剪影/棋盘格占位）

# ============ 分组渲染预设（每组一套完整参数，逐组独立精调）============
# 阶段0 诊断定稿（2026-06，编辑器可正常打开后重测，推翻旧"base color"方案）：
#   ① final color 可用、不会纯白屏；② EV bias 在本工程失效(EV0→14 画面无变化)；
#   ③ 打灯(sky/dir)会把伊瑟 NPR cel-shader 冲【过曝糊白】、丢色丢层次 → 统一【无灯】；
#   ④ 无灯 final + ACES(不flatten) + sat + shoulder，对几乎所有角色显色正确
#      （含 emissive 红角 / 眼睛 / 真实皮肤色 / NPR 描边，base 时代的 skin_fix hack 不再需要）；
#   ⑤ 暗角色(白甲/深色)无灯偏暗、EV 救不了 → 后处理【温和提亮】(只救很暗、正常亮度不动)。
# 三组(agt/cst/chr)起点参数一致、但各自独立副本，按组用 _XXX_TEST 小批分别精调。
# 字段：
#   capture          固定 "final"（截真实游戏渲染色）
#   sky/dir          灯光强度（统一 0=无灯；打灯会糊白，勿动）
#   ev               曝光 bias（本工程失效，留 -1 占位）
#   sat              饱和度（>1 增艳；用户定 1.4 鲜艳风格）
#   flatten/shoulder tonemap：flatten=False 走 ACES，shoulder 压高光防局部过曝
#   brighten_*       温和提亮：以前景 pct 分位为锚缩放到 target，scale∈[1,max]，只提不压
_FINAL = {
    "capture": "final", "sky": 0, "dir": 0, "dir_pitch": -30, "dir_yaw": -45,
    "ev": -1.0, "sat": 1.3, "flatten": False, "shoulder": 0.50,   # sat1.3：诊断挑定(1.4 肤色橙/1.0 肤色白,1.3 平衡);无灯唯一可行(打灯糊白)
    "brighten_pct": 45.0, "brighten_target": 135.0, "brighten_max": 2.5,
}
PRESETS = {
    "agt":  {**_FINAL},   # 探子(含 Friday)：多自发光/深色，偏暗的靠温和提亮
    "cst":  {**_FINAL},   # 装扮：数量最多，华丽配色
    "chr":  {**_FINAL},   # 主角/NPC：写实皮肤
    "prop": {**_FINAL},   # 道具/投掷物
}
...
def capture_source_for_mesh(mesh_path):
    # 统一 final color（真实游戏渲染色）。base color 方案已弃用（缺光照/发光/真实皮肤色）。
...
def get_fit_bounds(actor):
    """取景用【整体包围盒】（含武器/坐骑/披风等远伸件），保证"完整不缺件"。
    代价是带远伸件的角色主体偏小（用户已接受"完整优先、宽角色偏小"）。
    注：base 时代曾用碰撞体盒排除远伸件防显小，但导致"缺件"，展示级改回整体盒。"""
...
def get_or_create_capture(cam):
    ...
    # eden 实测：无灯 final color 显色正确、不糊白（打灯才糊白）。统一 final。
...
def shoot_one(mesh_path, mesh_name):
    ...
    # 统一截 final color（真实游戏渲染色，含光照/发光/NPR 描边），无灯，黑透明底
    ...
    w = unreal.EditorLevelLibrary.get_editor_world()
    # 单源：无灯 final color（实测无灯不糊白、显色正确；打灯才过曝）。
    ...
    try:
        ...
        pass
    finalize(groups, preset.get("brighten_target", 135.0), preset.get("brighten_max", 2.5),
             preset.get("brighten_pct", 45.0))
...
TRANSPARENT_BG = False      # False=合成灰底 (96,96,96) 直出 RGB（用户要灰底背景）；True=输出透明 RGBA
GRAY_BG = (96, 96, 96)


def finalize(groups, target=135.0, maxscale=2.5, pct=45.0):
    """groups: [(out_path, [tmp]), ...]，单源 final 截图。
    ① 反相 alpha + 反预乘 RGB（PropagateAlpha=2 约定：导出 alpha=coverage）。
    ② 温和提亮：以前景(不透明区)亮度 pct 分位为锚缩放到 ~target，scale∈[1,maxscale]，只提不压。
    ③ TRANSPARENT_BG=True→输出透明 RGBA；False→合成灰底 (96,96,96) 输出 RGB。
    target/maxscale/pct 由各组 preset 传入，分组独立调。删临时文件。
    """
    ...
    spec = json.dumps({"pairs": pairs, "target": float(target), "maxscale": float(maxscale),
                       "pct": float(pct), "transparent": TRANSPARENT_BG, "bg": list(GRAY_BG)},
                      ensure_ascii=False)
    script = (
        "import sys, json, os\n"
        ...
        "transparent=bool(d['transparent']); bg=np.array(d['bg'],dtype='float32')\n"
        "for out, tmps in d['pairs']:\n"
        "    src = next((t for t in tmps if os.path.exists(t)), None)\n"
        "    if src is None:\n"
        "        continue\n"
        "    arr = np.asarray(Image.open(src).convert('RGBA')).astype('float32')\n"
        "    rgb = arr[..., :3].copy()\n"
        "    a = 255.0 - arr[..., 3]\n"                     # 反相：coverage → 不透明度
        "    af = a / 255.0\n"
        "    m = af > 0.003\n"
        ...
        "    if transparent:\n"
        "        Image.fromarray(np.dstack([rgb, a]).astype('uint8'), 'RGBA').save(out, optimize=True)\n"
        "    else:\n"
        "        comp = rgb * af[..., None] + bg[None, None, :] * (1.0 - af[..., None])\n"
        "        Image.fromarray(np.clip(comp,0,255).astype('uint8'), 'RGB').save(out, optimize=True)\n"
        "    for t in tmps:\n"
        "        try:\n"
        "            os.remove(t)\n"
        "        except Exception:\n"
