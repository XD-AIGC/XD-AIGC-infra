...
ROOT = sys.argv[1] if len(sys.argv) > 1 else r"D:/角色识别数据/伊瑟_UE4"
...
TYPE_WORDS = ("Agt", "Cst", "Chr", "Prop", "Thrown")
# Chr 下需要合并到同一角色 ID 的前缀（按伊瑟世界观，这些是有大量套装的角色基底）
CHR_MERGE_PREFIXES = ("Chr_Boy_Home", "Chr_Girl_Home", "Chr_Friday")


def parse_role(rel):
    """返回 (type, role_id)。
    路径前可能有任意层（如 ArtResources/Characters/），找到第一段 type 词为锚点。
    """
    parts = rel.split("/")
    typ_idx = next((i for i, p in enumerate(parts) if p in TYPE_WORDS), None)
    if typ_idx is None or typ_idx + 1 >= len(parts):
        return "?", parts[-2] if len(parts) >= 2 else parts[0]
    typ = parts[typ_idx]
    char_dir = parts[typ_idx + 1]
    if typ == "Chr":
        for prefix in CHR_MERGE_PREFIXES:
            if char_dir.startswith(prefix):
                return typ, prefix
    return typ, char_dir


def variant_label(rel, role_id):
    """从 mesh 最深目录名提取『去掉角色 ID 后剩下的套装/变体标签』。
    例：
      Cst/Cst_Cabala/Cst_Cabala_Hollow/SK_Cst_Cabala_Hollow_LOD0  →  Hollow
      Chr/Chr_Boy_Home_Eveningdress/Eveningdress_Blackgold/SK_..._Ed_Blackgold_LOD0  →  Ed_Blackgold
      Chr/Chr_Friday_Heromask/Heromask_Hero/SK_Agt_Friday_Heromask_Hero_LOD0  →  Heromask_Hero
      Agt/Agt_Alicorn/SK_Agt_Alicorn_LOD0  →  default
    """
    parts = rel.split("/")
    last = parts[-1].split(".")[0]  # SK_..._LOD0.SK_..._LOD0 → SK_..._LOD0
    base = last
    for p in ("SK_", "Sk_", "sk_"):
        if base.startswith(p):
            base = base[len(p):]
            break
    for s in ("_LOD0", "_Lod0", "_lod0"):
        if base.endswith(s):
            base = base[:-len(s)]
            break
    for p in ("Cst_", "Chr_", "Agt_", "Prop_"):
        if base.startswith(p):
            base = base[len(p):]
            break
    # 角色 ID 去掉类型前缀（Chr_Boy_Home → Boy_Home）
    role_short = role_id
    for p in ("Chr_", "Cst_", "Agt_", "Prop_"):
        if role_short.startswith(p):
            role_short = role_short[len(p):]
            break
    if base.startswith(role_short + "_"):
        base = base[len(role_short) + 1:]
    elif base == role_short:
        base = "default"
    return base or "default"


def scan(root):
    """扫目录，返回按角色 ID 聚合的 list。"""
    raw = []  # (rel_dir, [(src, label), ...])
    front = VIEWS[0][0]
    ...
    for rel, views in raw:
        typ, role_id = parse_role(rel)
        var = variant_label(rel, role_id)
        if role_id not in groups:
            groups[role_id] = {"roleId": role_id, "type": typ, "variants": []}
        groups[role_id]["variants"].append({"name": var, "rel": rel, "views": views})

    # 同 role 下 variant 重名（多个 default 或 *_1/_2 撞车）→ 用 rel 路径末段去重
    for g in groups.values():
        ...
        for name, vs in seen.items():
            if len(vs) > 1:
                for v in vs:
                    suffix = v["rel"].split("/")[-1].split(".")[0]
                    v["name"] = f"{name} [{suffix}]"

    items = list(groups.values())
    items.sort(key=lambda g: g["roleId"].lower())
    for g in items:
...
<header>
  ...
  <span class="pill" data-filter="Agt">Agt</span>
  <span class="pill" data-filter="Cst">Cst</span>
  <span class="pill" data-filter="Chr">Chr</span>
  <span class="pill" data-filter="multi">仅多套装</span>
...
function matches(g) {
  ...
  if (filter !== 'all' && filter !== 'multi' && g.type !== filter) return false;
...
function render() {
  ...
  grid.innerHTML = filtered.map((g, gi) => {
    ...
    const nvar = multi ? '<span class="nvar">' + g.variants.length + ' 套装</span>' : '';
