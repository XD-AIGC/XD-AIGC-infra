...
def main():
    ...
    log("=" * 60)
    if restart_needed:
        log(">>> ini 文件已修改。无头模式下：本次进程结束后【再无头跑一次本/后续脚本】即生效 <<<")
        log(">>> （eden 工程 GUI 打不开，全部走 run_ue.bat 无头跑；ini 改动本地用、勿提交 P4）")
        log(">>> 然后跑 02_test.py / 03_batch.py")
    else:
        log(">>> ini 已是正确配置，可以直接跑 02_test.py / 03_batch.py")
